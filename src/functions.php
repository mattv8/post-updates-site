...
function implodeNotEmpty(string $separator, array $array): string
{
...
function fetchAllEntries($db_conn, $entryId)
{
    ...
    $FormsData = mysqli_stmt_get_result($stmt);
...
function fetchEventAttributes($db_conn, $eventId)
{
    ...
    mysqli_stmt_execute($stmt);
    $eventData = mysqli_stmt_get_result($stmt);
...
{
    // Build the query with prepared statement
    if ($keys === null) {
        $query = "SELECT COLUMN_NAME, COLUMN_COMMENT FROM information_schema.COLUMNS WHERE TABLE_NAME = ?";
        $stmt = mysqli_prepare($db_conn, $query);
        mysqli_stmt_bind_param($stmt, 's', $table);
    } else {
        $placeholders = implode(',', array_fill(0, count($keys), '?'));
        $query = "SELECT COLUMN_NAME, COLUMN_COMMENT FROM information_schema.COLUMNS WHERE TABLE_NAME = ? AND COLUMN_NAME IN ($placeholders)";
        $stmt = mysqli_prepare($db_conn, $query);
        $types = 's' . str_repeat('s', count($keys));
 ...
 */
function executeQuery(\mysqli $connection, string $query): \mysqli_result|false
{
 ...
 * @return void
 */
function handleQueryError(\mysqli $connection): void
{
...
function getAutoGeneratedColumns(\mysqli $db_conn, string $tableName): array
{
    ...
    $stmt = mysqli_prepare($db_conn, $autoGeneratedColumnsQuery);
    mysqli_stmt_bind_param($stmt, 's', $tableName);
    mysqli_stmt_execute($stmt);
    $autoGeneratedColumnsResult = mysqli_stmt_get_result($stmt);
 ...
 */
function breakStringAtSpace(string $string, int $maxLength): array
{
    $chunks = [];
